...
for i in code:
    ...
    if i.startswith("-"):
        ins.append(curr_ins)
        adr = i.split(":")[0].strip().replace("-", "").strip()
        name = i.split(":")[1].strip().split(" - ")[0].strip()
        curr_ins = {"adr": adr, "name": name, "steps": []}
    else:
        step_num = i.split(". ")[0].strip()
        if step_num != "0" and len(curr_ins["steps"]) == 0:
            curr_ins["steps"].append(
                {"step_num": "0", "signals": [SIGNAL_MAP["FETCH"]]})
        steps = i.split(". ")[1].replace("`", "").strip().split(", ")
        step_signals = [SIGNAL_MAP[s] for s in steps]
        curr_ins["steps"].append(
            {"step_num": step_num, "signals": step_signals})
import json
print(json.dumps(ins, indent=4))
