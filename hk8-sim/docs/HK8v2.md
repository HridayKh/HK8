...
- currently there is a issue
- if we change port to get data from another port and chabge port using `PORTI` or `PORTR`
  - the program counter will continue execution from that port instead of the one currently selected
- solution: add a seprate minor "bus" connecting only the ports
- the bus can be handeled with second por select register similar to psr, called per (port execute register)
- we change per to change where code executes and psr for data only
