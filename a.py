isa = """- 00: LDI - get from memory to register (immideate value for address)
- 01: LDR - get from memory to register (address from register)
- 02: STI - store to memory from register (immideate value for address)
- 03: STR - store to memory from register (address from register)
- 04: PORTI - set port from immideate value
- 05: PORTR - set port from register value
- 06: PORTS - store current port to register
- 07: COPY - copy value from one register to another register
...
- 57: NOP - no operation
- 58: HALT - stop the clock""".splitlines()

out = ""

for i in isa:
	a, b = i.split(" - ")[0].split(": ")
	opcode = a.split("- ")[1].strip()
	name = b.strip()
	out += f"{opcode},{name},{' '*(4-len(name))},{'yes' if 'immideate' in b else 'no'}\n"
